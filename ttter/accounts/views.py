...
class FollowerListView(LoginRequiredMixin, TemplateView):
    ...
    def get_context_data(self, **kwargs):
        ...
        ctx["followers"] = [follower for follower in self.user.all().prefetch_related("followees")]
...
class FollowingListView(LoginRequiredMixin, TemplateView):
    ...
    def get_context_data(self, **kwargs):
        ctx = super().get_context_data(**kwargs)
        self.user = get_object_or_404(User, username=self.kwargs["username"])
        ctx["followings"] = [follower for follower in self.user.all().prefetch_related("followers")]
        return ctx
